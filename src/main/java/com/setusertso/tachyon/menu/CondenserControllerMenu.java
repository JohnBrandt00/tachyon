...
public class CondenserControllerMenu extends AbstractContainerMenu {
    ...
    // Client constructor
    public CondenserControllerMenu(int containerId, Inventory playerInventory) {
        this(containerId, playerInventory, new ItemStackHandler(1), new ItemStackHandler(3), new SimpleContainerData(6));
    }

    // Server constructor
    public CondenserControllerMenu(int containerId, Inventory playerInventory,
                                     IItemHandler handler, IItemHandler upgradeHandler, ContainerData data) {
        super(ModMenuTypes.CONDENSER_CONTROLLER.get(), containerId);
        this.data = data;

        // Input slot (tachyon shards only)
